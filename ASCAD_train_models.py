...
from keras.layers import Flatten, Dense, Input, Conv1D, MaxPooling1D, GlobalAveragePooling1D, GlobalMaxPooling1D, AveragePooling1D, BatchNormalization, Activation, Add, add
...
# Bug fix for some Nvidia 2080 rtx card with previous versions of Tensorflow (<2.0)
# This bug fix is deactivated by default and can be activated with the parameter nvidia_bugfix
def bugfix_nvidia2080rtx():
...
def resnet_v1(input_shape, depth, num_classes=256, without_permind=0):
	if (depth - 2) % 18 != 0:
		raise ValueError('depth should be 18n+2 (eg 20, 38, 56 ...)')
...
def train_model(X_profiling, Y_profiling, model, save_file_name, epochs=150, batch_size=100, multilabel=0, validation_split=0):
...
def read_parameters_from_file(param_filename):
	...
	nvidia_bugfix = 0
	if ("nvidia_bugfix" in my_parameters):
		nvidia_bugfix = my_parameters["nvidia_bugfix"]
	return ascad_database, training_model, network_type, epochs, batch_size, train_len, validation_split, multilabel, nvidia_bugfix
...
if __name__ == "__main__":
	if len(sys.argv)!=2:
		...
		ascad_database, training_model, network_type, epochs, batch_size, train_len, validation_split, multilabel, bugfix = read_parameters_from_file(sys.argv[1])

	if bugfix==1:
		print("nvidia debug")
		bugfix_nvidia2080rtx()
	...
	if(network_type=="mlp"):
		...
		best_model = resnet_v1((15000,1), 20)
	elif(network_type=="multi_resnet_without_permind"):
		best_model = resnet_v1((15000,1), 20, without_permind=1)
